...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        RequestManager(this@MainActivity).GetAllQuotes(listner)
        dialog= ProgressDialog(this@MainActivity)
        dialog?.setTitle("Loading...")
        dialog?.show()

    }
    private val listner:QuotesResponseListner=object :QuotesResponseListner{
...
private val copyListener:CopyListner=object :CopyListner{
    override fun onCopyClicked(text: String) {
     val clipboard=getSystemService(CLIPBOARD_SERVICE)as ClipboardManager
     val clip=ClipData.newPlainText("copied data",text)
     clipboard.setPrimaryClip(clip)
     Toast.makeText(this@MainActivity,"Quotes Copied to Clipboard !",Toast.LENGTH_LONG).show()
    }
